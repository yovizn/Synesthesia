...
import { editEventAction } from '@/utils/action/editEventAction'
...
export default function EditEventForm({ data }: { data: EventDetailType }) {
  ...
  const form = useForm<EditEventType>({
    ...
    defaultValues: {
      ...
      priceReguler: Number(data.Tickets.find((val) => val?.type === 'REGULER')?.price!),
      capacityReguler: Number(data.Tickets.find((val) => val?.type === 'REGULER')?.capacity!),
      priceVip: Number(data.Tickets.find((val) => val?.type === 'VIP')?.price!),
      capacityVip: Number(data.Tickets.find((val) => val?.type === 'VIP')?.capacity!),
      use_voucher: false,
    },
    mode: 'onChange',
  })
  ...
  const onSubmit = async (payload: EditEventType) => {
    try {
      const action = await editEventAction(payload, data.id)
      toast({
              ...
              render={({ field: { value, ...fieldValues }, formState }) => (
                <FormItem>
                  <FormLabel className="group mx-auto flex w-fit cursor-pointer flex-col items-center justify-center gap-2">
                    <Image
                      src={
                        form.getValues('avatar')
                          ? window.URL.createObjectURL(form.getValues('avatar')!)
                          : data.poster?.name
                            ? renderImage.png(data.poster?.name)
                            : placeholder
                      }
                      alt="Profile image"
                      fill
                    ...
                    />
                    {!form.getValues('avatar') ? (
                      <span className="absolute left-1/2 top-1/2 z-10 flex size-fit -translate-x-1/2 flex-col items-center justify-center gap-2 rounded-md bg-background/50 p-4">
                        <PlusCircle className="size-6 text-foreground md:size-10" />
                        <span className="block text-foreground">Upload your Image/Poster/Banner</span>
                      </span>
                    ) : null}
                  </FormLabel>
                  <FormControl>
